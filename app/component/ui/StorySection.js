    ...
    async function fetchStories() {
      try {
        ...
        const [usersData, postsData] = await Promise.all([
          get("/users?limit=12&select=id,firstName,lastName,username,image"),
          get("/posts?limit=12"),
        ]);
...
function SkeletonCard() {
  return (
    <div className="relative min-w-28 h-48 rounded-[22px] flex-shrink-0 overflow-hidden bg-white/5 animate-pulse">
      {/* fake avatar */}
...
function StoryViewer({ stories, startIndex, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="relative w-85 h-150 rounded-4xl overflow-hidden shadow-2xl"
      ...
      >
        ...
        <div className="absolute top-4 left-4 right-4 flex gap-1">
          {stories.map((_, i) => (
            <div
              key={i}
              className="flex-1 h-0.5 bg-brand/50 rounded-full overflow-hidden"
            >
              <div
        ...
        <div className="absolute top-8 left-4 right-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="w-9 h-9 rounded-full p-0.5 bg-secondary/50">
              <Image
                src={story.image}
                alt={story.name}
                width={36}
...
function UploadModal({ onClose, onPost }) {
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-sm mx-4 rounded-[28px] border border-white/10 bg-brand/5 shadow-2xl overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        {/* header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/[0.07]">
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center text-primary/50 hover:text-white transition-all"
          >
            <X size={15} />
          </button>
        </div>
        ...
        <div className="p-5 space-y-4">
            ...
            >
              ...
              <div className="text-center">
                ...
                <p className="text-gray-500 text-xs mt-1">
                  or{" "}
                  <span className="text-secondary/80 underline underline-offset-2">
                    browse
                  </span>{" "}
                  to choose
                </p>
              ...
              <div className="flex gap-2 mt-1">
                {["JPG", "PNG", "GIF", "MP4", "MOV"].map((ext) => (
                  <span
                    key={ext}
                    className="text-[10px] text-primary/80 bg-white/5 border border-white/10 rounded px-1.5 py-0.5"
                  >
                    {ext}
                  </span>
                ))}
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={
                preview ? handlePost : () => fileInputRef.current?.click()
              }
              disabled={posting}
